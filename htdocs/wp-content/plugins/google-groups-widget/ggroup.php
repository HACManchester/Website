...
/*	function activate(){
	  ...
	  function widget($args, $instance){
		...
		$size = empty($instance['size']) ? '7' : $instance['size'];
		define('MYPLUGIN_PATH', get_settings('siteurl').'/wp-content/plugins/google-groups-widget');
		// Before the widget
		echo $before_widget;
...
//			echo('<img src="'.PLUGINDIR.'/ggroups/googlegroups.png"><br />');
		...
		echo('<div id="gtext">');
			...
			$rss = fetch_feed('http://groups.google.com/group/'.$group.'/feed/rss_v2_0_msgs.xml');
			if ( ! is_wp_error( $rss ) ) {
    				$maxitems = $rss->get_item_quantity( $size ); 
    				$rss_items = $rss->get_items( 0, $maxitems );
				foreach ( $rss_items as $item ) {
					echo '<li style="clear:both; padding: 5px 0 15px 0;">';
					echo '<div style="padding: 0px 0px 2px 0px; border-bottom: 1px solid #f0f0f0;"><a href="'.esc_url( $item->get_permalink() )."\" title=\"".esc_html( $item->get_description() )."\">".esc_html( $item->get_title() )."</a></div>";
					echo '<div style="float: left; margin-top: 3px; font-size: 0.9em; color: grey;">'.esc_html( $item->get_date() ).'</div>';
					echo '</li>';
				}
			}
			echo('</ul>');
